(ns sistema-gpus-core.api.arquiteturas
  (:require
...
(def controller (ctrl/->arquiteturas-controller))
...
;; {:id_arquitetura <valor>} como clause.  
(defn get-by-id [{{:keys [id]} :params}]
  (try
    (let [result (ctrl-client/get-item controller {:id_arquitetura id})]
      (if (empty? result)
        (no-content)
        (ok result)))
    (catch Exception e
...
;; A clause deve ser {:id_arquitetura id}, e updates é o body.
(defn put-item [{{:keys [id]} :params entity :body}]
  (try
    (let [result (ctrl-client/update-item controller entity {:id_arquitetura id})]
      (ok result))
    (catch Exception e
      (log/error (.getMessage e))
      (bad-request (.getMessage e)))))
...
(defroutes arquiteturas-routes
  (context "/api" []
    (context "/arquiteturas" []
      (GET "/" [] get-all)
      (GET "/:id" [id] get-by-id)
      (POST "/" [] post-item)
      (PUT "/:id" [id] put-item)
