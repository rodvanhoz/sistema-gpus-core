(ns sistema-gpus-core.api.processadores
  (:require
...
(def controller (ctrl/->processadores-controller))

  ;; Função GET /api/id_processador
(defn get-all [_]
  (try
...
;; Função GET /api/id_processador/:id
  ;;
  ;; Observação: para compatibilidade com o controller, passamos
  ;; {:id_processador <valor>} como clause.  
(defn get-by-id [{{:keys [id]} :params}]
  (try
    (let [result (ctrl-client/get-item controller {:id_processador id})]
      (if (empty? result)
        (no-content)
        (ok result)))
    (catch Exception e
...
;; Função POST /api/id_processador
...
;; Função PUT /api/id_processador/:id
  ;;
  ;; Per your controller, update-item recebe (updates, clause).
  ;; A clause deve ser {:id_processador id}, e updates é o body.
(defn put-item [{{:keys [id]} :params entity :body}]
  (try
    (let [result (ctrl-client/update-item controller entity {:id_processador id})]
      (ok result))
    (catch Exception e
      (log/error (.getMessage e))
      (bad-request (.getMessage e)))))

;; Função DELETE /api/id_processador/:id
...
(defroutes processadores-routes
  (context "/api" []
    (context "/processadores" []
      (GET "/" [] get-all)
      (GET "/:id" [id] get-by-id)
      (POST "/" [] post-item)
      (PUT "/:id" [id] put-item)
