(ns sistema-gpus-core.controller.configuracoes-jogos
  (:require
   [sistema-gpus-core.domain.client :refer [ControllerClientProtocol]]
   [sistema-gpus-core.models.configuracoes-jogos :as model]
   [sistema-gpus-core.domain.model :refer :all]
   [sistema-gpus-core.helper.utils :refer [uuid-from-string]]
   [sistema-gpus-core.logic.configuracoes-jogos :as logic]
   [clojure.tools.logging :as log]))

(def model (model/->ConfiguracoesJogos))

      ;; record que implementa ControllerClientProtocol
(defrecord ConfiguracoesJogosController []
  ControllerClientProtocol
        ...
        ;; -------------------------
  (read-all [_]
    (let [raw-items (read-all (model/->ConfiguracaoJogoModel))]
  ...
  (items-count [_]
    (items-count model)))
      ...
      ;; construtor
(defn ->configuracoes-jogos-controller []
  (->ConfiguracoesJogosController))
