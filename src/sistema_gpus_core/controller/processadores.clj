...
(defrecord ProcessadoresController []
  ...
  (read-all [_]
    (let [raw-items (read-all model)]
      (log/info "Reading all processadores's")
      (map (fn [item]
  ...
  (get-item
    [_ clause]
    (log/info (format "processadores found with clause: %s" clause))
  ...
  (put-item [_ entity]
    (let [entity* (logic/prepare entity)]
      (log/info (format "Putting processadores: %s" entity*))
  ...
  (update-item [_ updates clause]
    (let [updates*  (logic/string-fields->object updates)
      ...
      (log/info (format "Updating processadores [clause: %s] with infos: %s" clause* updates*))
  ...
  (items-count [_]
    (items-count model)))
              ...
              ;; construtor
(defn ->processadores-controller []
  (->ProcessadoresController))
