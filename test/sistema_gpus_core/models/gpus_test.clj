(ns sistema-gpus-core.models.gpus-test
  (:require [clojure.test :refer :all]
            ...
            [sistema-gpus-core.helper.utils :refer [uuid-from-string uuid str->date]]))

;; 1) Usamos a fixture que sobe o container e aplica o baseline
(use-fixtures :once db-setup/with-postgres-testcontainer)

;; ;; 2) Podemos usar outra fixture para inserir mock de GPUs antes de cada test
;; (use-fixtures :each
;;   (fn [f]
...
(deftest test-gpus-crud
  ...
  (testing "read-all deve retornar as GPUs mockadas"
    (let [result (model/read-all (gpus/->GPU))]
  ...
  (testing "get-item retorna a GPU esperada"
    (let [gpu-id (uuid-from-string "669e6eb0-6528-489a-ac90-bb3670cb6a78")
          gpu    (model/get-item (gpus/->GPU) :id_gpu gpu-id)]
  ...
  (testing "put-item! insere uma nova GPU"
    (let [new-gpu {:nome_fabricante          "ASUS"
          ...
          _      (model/put-item! (gpus/->GPU) new-gpu)
          result (model/get-item (gpus/->GPU) :nome_modelo "ASUS SPECIAL GPU BLA")]
...
;; Testar update-item!
  (testing "update-item! atualiza um campo da GPU"
    (let [gpu-id  (uuid-from-string "669e6eb0-6528-489a-ac90-bb3670cb6a78")
          _       (model/update-item! (gpus/->GPU) {:id_gpu gpu-id} :nome_modelo "GeForce GTX 770 UPDATED")
          updated (model/get-item (gpus/->GPU) :id_gpu gpu-id)]
  ...
  (testing "delete-item! remove uma GPU do banco"
    (let [gpu-id (uuid-from-string "a41adbc8-acf0-432e-bd13-a2eb7d66b034")]
      (model/delete-item! (gpus/->GPU) gpu-id)
      (is (nil? (model/get-item (gpus/->GPU) :id_gpu gpu-id)))))

  ;; Testar items-count
  (testing "items-count retorna a contagem de GPUs"
    (let [counter (model/items-count (gpus/->GPU))]
